<div class="navbar navbar-inner block-header">
	<div class="muted pull-left">Informasi Dokumen (Berita Acara Pemusnahan Dokumen, Sosialisasi & Review Dokumen) </div>
</div>
...
$tglhrini = date("Y-m-d");
/*
$e1 = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM distribusidok a 
									LEFT JOIN ddist b ON a.uid=b.uid 
...
$e = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM distribusidok a 
									...
									WHERE b.pId='$_SESSION[cv]' AND b.pdid=$_GET[pdid] AND a.suid=$_GET[id]"));
/*}
else {
$e = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM distribusidok a 
									LEFT JOIN ddist b ON a.suid=b.suid 
...
if ($e['psACC']=='N'){
	$status = "Belum selesai ditindaklanjuti";
}else{
	$status = "Telah dibaca dan dintindaklanjuti";
}
...
mysql_query("UPDATE ddist SET psTglbaca='$tglhrini' WHERE pdid='$e[pdid]' AND cId='$_SESSION[cv]'");
}
elseif ($e['psTglbaca']=='' AND $e[jawab]=="N") {
mysql_query("UPDATE ddist SET psTglbaca='$tglhrini', psTglselesai='$tglhrini', psACC='Y' WHERE pdid='$e[pdid]' AND cId='$_SESSION[cv]'");
}
?>
<strong>
...
<div class="span12">
	...
	<?php
		...
		$dsp = mysql_query("SELECT a.*,b.cNama, b.cIdjab FROM ddist a LEFT JOIN users b ON a.pId=b.cId WHERE a.pId='$_SESSION[cv]'  ORDER BY a.ptgl DESC");
		...
		if ($s['psTglbaca']==''){
					echo"<td><a href='?pages=ddist2&act=detail&id=$s[suid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Baca</a>
					</td>
			  </tr>";	
		}
			  else {
				  echo"<td><a href='?pages=ddist2&act=detail&id=$s[suid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Detail</a>
				  </td>
			  </tr>";
