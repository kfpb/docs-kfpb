<div class="navbar navbar-inner block-header">
	<div class="muted pull-left">Informasi Hasil Review Dokumen</div>
</div>
...
$e1 = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM disposisidok a 
									...
									WHERE b.pId='$_SESSION[cv]' AND pdid=$_GET[pdid] AND a.uid=$_GET[id]"));
...
<div class="span12">
	...
	<?php
		...
		$dsp = mysql_query("SELECT a.*,b.cNama, b.cIdjab FROM ddis a LEFT JOIN users b ON a.pid=b.cId WHERE a.pId='$_SESSION[cv]'  ORDER BY a.ptgl DESC");
		...
		if ($s['psTglbaca']==''){
					echo"<td><a href='?pages=ddis2&act=detail&id=$s[suid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Baca</a>
					</td>
			  </tr>";	
		}
			  else {
				  echo"<td><a href='?pages=ddis2&act=detail&id=$s[suid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Detail</a>
				  </td>
			  </tr>";
