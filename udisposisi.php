...
if($_GET[act]=="detail"){
$tglhrini = date("Y-m-d");
$e1 = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM disposisi a 
...
mysql_query("UPDATE pdis SET psTglbaca='$tglhrini', psTglselesai='$tglhrini', psACC='Y' WHERE pdid='$e[pdid]' AND cId='$_SESSION[cv]'");
}
?>
<strong>
<legend>Detail Disposisi</legend></strong>
...
if ($e1[iid]==0) {
	...
	$memo = mysql_fetch_array(mysql_query("SELECT * FROM sinter WHERE siid='$e[siid]'"));
	
    echo"
	<tr><td>Lihat Memo</td><td>: <strong>
    <a href='home.php?pages=sinter&act=detail&id=$e[siid]'>Klik disini lihat detail memo/undangan</a> <br>: Perihal = $memo[siperihal]
	</strong></td></tr>
	<tr><td>Instruksi/Informasi :</td><td>$e[pInstruksi]</td></tr>
	<tr><td>Lampiran Disposisi :</td><td><a href='disposisi/$e[disfile]'>Klik disini (jika ada)</a></td></tr>
	<tr><td>Jawaban/Informasi Anda :</td><td>$e[info]</td></tr>
</table>
</strong>
<br>";
	
if ($e[psACC]=="N" AND $e[jawab]=="Y"){
$e = mysql_fetch_array(mysql_query("SELECT * FROM pdis WHERE siid=$_GET[id] AND pdid=$_GET[pdid]"));	
$ed = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE cId='$e[cId]'"));
	echo"<form method='post' action='?pages=udis&act=accm&pdid=$e[pdid]&siid=$e[siid]' enctype='multipart/form-data'>
...
$f = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM disposisi a 
									LEFT JOIN pdis b ON a.siid=b.siid 
									LEFT JOIN users c ON b.pid=c.cId 
									LEFT JOIN sinter d ON a.siid=d.siid
									WHERE b.cId='$_SESSION[cv]' AND pdid=$_GET[pdid] AND a.siid=$_GET[id]"));
...
$fgh = mysql_fetch_array(mysql_query("SELECT * FROM disposisi WHERE dPendisposisi='$e[pId]' AND siid='$_GET[id]'"));

$qds = mysql_query("SELECT a.*,(SELECT cNama FROM users WHERE cId=$e[pId]) as dPdisposisi FROM disposisi a WHERE a.siid='$_GET[id]'");
...
$pds = mysql_query("SELECT a.*,
					...
					FROM pdis a WHERE a.siid='$_GET[id]' AND a.pId='$e[pId]' ORDER BY a.pdid DESC");
...
/*$nama = mysql_fetch_array(mysql_query("SELECT cNama FROM users WHERE cId=$e[ipengirim]"));
  ...
  */
	...
	<tr><td>Lampiran Disposisi :</td><td><a href='disposisi/$e[disfile]'>Klik disini (jika ada)</a>
	</td></tr>
   		<tr><td>Instruksi/Informasi :</td><td>$e[pInstruksi]</td></tr>
	<tr><td>Jawaban/Informasi Anda :</td><td>$e[info]</td></tr>
...
if ($e[psACC]=="N" AND $e[jawab]=="Y"){
$e = mysql_fetch_array(mysql_query("SELECT * FROM pdis WHERE iid=$_GET[id] AND pdid=$_GET[pdid]"));	
$ed = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE cId='$e[cId]'"));	
	
	echo"<form method='post' action='?pages=udis&act=acc&pdid=$e[pdid]&iid=$e[iid]' enctype='multipart/form-data'>
	<div class='control-group'>
		<label class='control-label' for='info'><b>Jawaban/ Informasi Penyelesaian : (Wajib dijawab) </b></label>
        <div class='controls'>
...
$fg = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE cNama='$e[cNama]'"));
$fgh = mysql_fetch_array(mysql_query("SELECT * FROM disposisi WHERE dPendisposisi='$e[pId]' AND iid='$_GET[id]'"));

$qds = mysql_query("SELECT a.*,(SELECT cNama FROM users WHERE cId=$e[pId]) as dPdisposisi FROM disposisi a WHERE a.iid='$_GET[id]'");
$ds = mysql_fetch_array($qds);
$jds = mysql_num_rows($qds);
...
$pds = mysql_query("SELECT a.*,
					...
					FROM pdis a WHERE a.iid='$_GET[id]' AND a.pId='$e[pId]' ORDER BY a.pdid DESC");
...
function UploadJwb($fupload_name){
 ...
 if($_FILES['fupload']['size']<=$maxsize){
	$tglhrini = date("Y-m-d");
	mysql_query("UPDATE pdis SET info='$_POST[info]', psTglselesai='$tglhrini', psACC='Y', filedis='$nama_file_unik' WHERE pdid='$_GET[pdid]' AND cId='$_SESSION[cv]'");
	
	mysql_query("UPDATE isurat SET istatus='N' WHERE iid='$_POST[iid]'");

	echo "<script>window.location=('home.php?pages=udis&act=detail&pdid=$_GET[pdid]&id=$_GET[iid]');</script>";
...
function UploadJwb($fupload_name){
 ...
 if($_FILES['fupload']['size']<=$maxsize){
	...
	echo "<script>window.location=('home.php?pages=udis&act=detail&pdid=$_GET[pdid]&id=$_GET[siid]');</script>";
 }
  else {
	   echo "<script>window.alert('Maaf... file yang ada pilih terlalu besar, maksimal 15 MB..!');self.history.back();</script>";
  }  	
...
<div class="span12">
	...
	<?php
		...
		$dsp = mysql_query("SELECT a.*,b.cNama, b.cIdjab, b.cJabatan FROM pdis a LEFT JOIN users b ON a.pid=b.cId WHERE a.cId='$_SESSION[cv]' ORDER BY a.ptgl DESC");
		while($s = mysql_fetch_array($dsp)) {
			$p = mysql_fetch_array (mysql_query("SELECT * FROM sinter WHERE siid='$s[siid]'"));
			...
			$tglS=$s[ptgls];
			if ($s[ptgls]=="0000-00-00" OR $s[ptgls]==NULL){
				$tglS="-";
			}
		...
		if ($s[psTglbaca]=='0000-00-00'){
					echo"<td><a href='?pages=udis&act=detail&id=$s[siid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Baca</a>
					<a href='home1.php?pages=udis&act=detail&id=$s[siid]&pdid=$s[pdid]' title='print' class='btn btn-info'><i class='icon-print'></i></a>
					</td>
			  </tr>";	
		}
			  else {
				  echo"<td><a href='?pages=udis&act=detail&id=$s[siid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Detail</a>
					<a href='home1.php?pages=udis&act=detail&id=$s[siid]&pdid=$s[pdid]' title='print' class='btn btn-info'><i class='icon-print'></i></a>
				  </td>
			  </tr>";
			  ...
			  if ($s[psTglbaca]=='0000-00-00'){
					echo"<td><a href='?pages=udis&act=detail&id=$s[iid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Baca</a>
					<a href='home1.php?pages=udis&act=detail&id=$s[iid]&pdid=$s[pdid]' title='print' class='btn btn-info'><i class='icon-print'></i></a>
					</td>
			  </tr>";	}
			  else {
				  echo"<td><a href='?pages=udis&act=detail&id=$s[iid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Detail</a>
					<a href='home1.php?pages=udis&act=detail&id=$s[iid]&pdid=$s[pdid]' title='print' class='btn btn-info'><i class='icon-print'></i></a>
				  </td>
			  </tr>";
	...
	Klik Baca/Detail untuk melihat Detail dan Konfirmasi Telah Dibaca serta menjawab penyelesaian Disposisi</h5>
	</span>
</div>
</div>
<?php
