...
header("Content-Disposition: attachment; filename=Data_SPPTek.xls");
	...
	<thead>
		...
		<th>No</th>
			...
			<th>No SPPTek</th>
			<th>Perihal</th>
			<th>Supervisor Teknik</th>
			<th>Kode Lokasi</th>
			<th>Nama Lokasi (Baru)</th>
			<th>No Aktiva (Baru)</th>
			<th>Nama Aktiva</th>
			<th>Keluhan (Lama)</th>
			<th>Keluhan (Baru)</th>
			<th>Kontak Person (Baru)</th>
        	<th>Di Cek Oleh</th>
			<th>No Order/PR</th>
			<th>Pihak ke-3?</th>
...
if ($_POST[jenispptek]=='ALL'){
	...
	$smasuk = mysql_query("SELECT a.*,b.*,c.cIdjab, c.cJabatan FROM sinter a LEFT JOIN psin b ON a.siid=b.siid LEFT JOIN users c ON a.sipengirim2=c.cId  WHERE b.cId='80' && a.sstatus='Y' && jenisms = 20 && jenispptek='$_POST[jenispptek]' ORDER BY a.sitgl DESC");
}
		$no = 1;	
		...
		echo"<td width=5%>";echo tgl_indo($s[sitgl]);echo"</td>
				...
				<td><font size=2>$s[siperihal]</font></td>
				<td>$s[jenispptek]</td>";
				
			if ($loc2[nama_area]!='')
		       {
				echo"<td>$s[lokasi]</td>
	     	    <td>$loc2[nama_area]</td>";
		       }
		       else
		       {
				echo"<td></td>
			...
			if ($akt2[aknama]!='')
		       ...
		       else
		       {
				...
				<td>$s[keluhan]</td>
				<td>$s[personil]</td>
				<td>$s[siket_teknik]</td>
				<td>$s[sikomen]</td>
				<td>$s[pihak3]</td>
				<td>$s[wp]</td>
				<td>$s[sitgl_cek]</td>
				<td>$s[sitgl_order]</td>	
				<td>$s[sitgl_brgdtg]</td>
				<td>$s[sitgl_mulai]</td>
				<td>$s[sitgl_selesai]</td>
				<td>$s[sitgl_rework]</td>	
				<td>$s[sikomen2]</td>
				</tr>";	
				$no++;
		}
