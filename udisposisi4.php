<div class="navbar navbar-inner block-header">
	<div class="muted pull-left">[ALL] Semua Tindak Lanjut (RTL) CAPA</div>
</div>
...
$e1 = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM disposisi2 a 
									...
									WHERE b.pdid=$_GET[pdid] AND a.iid=$_GET[id]"));
...
<table width="100%" border=1>
    ...
    <tr><td>Tanggal Buat CAPA</td><td>: <?=tgl_indo($e[ptgl]);?></td></tr>
	<tr><td>Tanggal Batas Waktu</td><td>: <?=tgl_indo($tglS);?></td></tr>
    <tr>
		<td>Pembuat</td>
...
/*$nama = mysql_fetch_array(mysql_query("SELECT cNama FROM users WHERE cId=$e[ipengirim]"));
  ...
  */
	...
	echo"
	<tr><td>Tugas/CAPA :</td><td>$e[pInstruksi]</td></tr>
	<tr><td>Lihat Sumber Tugas/CAPA</td><td>: <strong>
    <a href='home.php?pages=suin2&act=detail&id=$e[iid]'>Klik disini Lihat Sumber Tugas/CAPA</a><br>
...
$ed = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE cId='$e[cId]'"));	
	...
	echo"<form method='post' action='?pages=udis4&act=acc&pdid=$e[pdid]&iid=$e[iid]' enctype='multipart/form-data'>";
...
function UploadJwb($fupload_name){
 ...
 if($_FILES['fupload']['size']<=$maxsize){
	$tglhrini = date("Y-m-d");
	mysql_query("UPDATE pdiss SET ptgls='$_POST[tgls]', temuan='$_POST[temuan]', kondisi='$_POST[kondisi]',syarat='$_POST[syarat]',gap='$_POST[gap]',rootcause='$_POST[rootcause]', info='$_POST[info]', psTglselesai='$tglhrini', psACC='Y', filedis='$nama_file_unik' WHERE pdid='$_GET[pdid]'");
	
	mysql_query("UPDATE ssurat SET istatus='N' WHERE iid='$_POST[iid]'");

	echo "<script>window.location=('home.php?pages=udis4&act=detail&pdid=$_GET[pdid]&id=$_GET[iid]');</script>";
...
<div>
<div>
      <form method="post" action='home1.php?pages=udisposisi4_export' target=_blank>
    <input class="btn btn-primary" type="submit" value="Export" /></form>
	<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="Tb14" width=120%>
	<thead>
		<tr><th width=1%></th>
			<th>Sumber CAPA</th>
			<th width=5%>Tgl Buat/Audit</th>
		    <th width=5%>Temuan</th>
		    <th width=5%>Persyaratan</th>
		    <th width=5%>Kondisi Saat Ini</th> 
		    <th width=5%>GAP Analisis</th> 
		    <th width=5%>RootCause Analisis</th> 	    
			<th>CAPA</th>
	...
	<?php
		...
		$dsp = mysql_query("SELECT a.*,b.cNama, b.cIdjab, b.cJabatan FROM pdiss a LEFT JOIN users b ON a.cid=b.cId WHERE a.siid=0 ORDER BY a.ptgls DESC");
		while($s = mysql_fetch_array($dsp)) {
            $dsp2 = mysql_query("SELECT a.*,b.cNama, b.cIdjab, b.cJabatan FROM pdiss a LEFT JOIN users b ON a.cid=b.cId WHERE a.siid=0 ORDER BY a.ptgls DESC");
            $s2 = mysql_fetch_array($dsp2);
			$p1 = mysql_fetch_array (mysql_query("SELECT * FROM ssurat WHERE iid='$s[iid]'"));
			$sft = Array("14"=>"Improve","16"=>"TargetMutu","22"=>"Risk","23"=>"K.Pelanggan","29"=>"NCP","6b"=>"Audit","6c"=>"Reg","11"=>"Lain2","21"=>"Rapat");
			$bdg = Array("A"=>"success","B"=>"warning","C"=>"important");
			...
			echo	"<td>$s[psACC]</td>
			  ...
			  if ($s[psTglbaca]=='0000-00-00'){
					echo"<td><a href='?pages=udis4&act=detail&id=$s[iid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Baca</a>
				
					</td>
			  </tr>";	}
			  else {
				  echo"<td><a href='?pages=udis4&act=detail&id=$s[iid]&pdid=$s[pdid]' title='Baca/Detail' class='btn btn-info'>Detail</a>
				  </td>
			  </tr>";
