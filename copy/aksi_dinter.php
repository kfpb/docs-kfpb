...
if ($act=='tambah'){
  ...
  UploadDinter($nama_file_unik);
if($_FILES['fupload']['size']<=$maxsize){
if (empty($lokasi_file)){
		 $q=mysql_query("INSERT INTO dinter(ditgl,
                                   dipengirim,
                                   dikodok,
								   direv,
								   dikodok1,
...
Admin E-kfpb (Firman)')");
*/
echo "<script>window.alert('Distribusi Dokumen tersimpan dan menunggu ACC SPD-MR');window.location=('../../home.php?pages=dinter')</script>
...
Admin E-kfpb (Firman)
");
if($mail->Send()) echo "<center><h4>Memo/Undangan berhasil tersimpan dan email terkirim ke atasan langsung, <br><a href=../../home.php?pages=dinter>KLIK DISINI</a> untuk kembali!
</h4></center>";
else echo "<center><h4>Memo/Undangan berhasil tersimpan tetapi email ke atasan langsung gagal terkirim, <br><a href=../../home.php?pages=dinter>KLIK DISINI</a> untuk kembali!
</h4></center>";

echo"
<script>
        var timer = setTimeout(function() {
            window.location='../../home.php?pages=dinter'
        }, 3000);
</script>";
*/
...
if (empty($lokasi_file)){
     ...
     $q=mysql_query("UPDATE dinter SET ditgl 	  = '$_POST[tgl]',
...
unlink("../../distribusidok/$data[difile]"); 
  ...
  if ($q){
	  echo "<script>window.alert('Distribusi Dokumen Terupdate');window.location=('../../home.php?pages=dinter')</script>";
  }else{
...
$q=mysql_query("UPDATE dinter SET  dinmr 		 = '$newID',
								   distatus   	 = 'Y'
								   WHERE suid = '$_GET[id]'");

$e = mysql_fetch_array(mysql_query("SELECT * FROM dinter WHERE suid='$_GET[id]'"));
								   
$r=mysql_query("UPDATE dokumen SET  nama_file	 = '$e[difile]' WHERE kode_dok = '$e[dikodok]'"); 
...
$r = mysql_fetch_array(mysql_query("SELECT a.cUser,a.cNama,a.cIdjab,b.* FROM users a
						LEFT JOIN dinter b ON b.dipengirim1=a.cId
						WHERE b.suid='$_GET[id]'"));
...
*/ 
	  echo "<script>window.alert('Distribusi Dokumen Terkirim');window.location=('../../home.php?pages=dinter')</script>";


		
  }else{
	  echo "<script>window.alert(Distribusi Dokumen Gagal Terkirim!');self.history.back();</script>";
  }
}
...
elseif ($act=='hapus'){
 $data=mysql_fetch_array(mysql_query("SELECT difile,suid FROM dinter WHERE suid='$_GET[id]'"));
  ...
  echo "<script>window.location=('../../home.php?pages=dinter')</script>"; 
}

//tambah penerima dan tembusan
elseif ($act=='lp'){
  mysql_query("DELETE FROM dsin WHERE copyke=$_POST[copyke] AND suid='$_GET[id]'");
  $dsin = $_POST["dsin"];
  foreach ($dsin as $x=>$cid)
  {
	$q=mysql_query("INSERT INTO dsin(copyke,cId,suid) VALUES ('$_POST[copyke]','$cid','$_GET[id]')");  
  }
...
elseif ($act=='lp1'){
  ...
  if ($q){
	...
	echo "<script>window.alert('Penerima Salinan dihapus, ulangi lagi input penerima salinan');window.location=('../../home.php?pages=dinter')</script>";
  }else{
	  echo "<script>self.history.back();</script>";
  }
}
