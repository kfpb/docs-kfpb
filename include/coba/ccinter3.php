...
$e = mysql_fetch_array(mysql_query("SELECT a.*,b.*,c.cNama,c.cFoto,d.* FROM rtcc a 
									...
									WHERE b.cId='$_SESSION[cv]' AND pdid=60 AND a.ccid=$_GET[id] OR b.cId='$_SESSION[cv]' AND pdid=49 AND a.ccid=$_GET[id]"));
									
$ed = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE cNama='$e[cNama]'"));
$edf = mysql_fetch_array(mysql_query("SELECT * FROM rtcc WHERE dPendisposisi='60' AND ccid='$_GET[id]' OR dPendisposisi='49' AND ccid='$_GET[id]'"));
...
$pds0 = mysql_query("SELECT a.*,
					...
					FROM cdis a WHERE a.ccid='$_GET[id]' AND a.pId='49' OR a.ccid='$_GET[id]' AND a.pId='60' ORDER BY a.pdid DESC");
