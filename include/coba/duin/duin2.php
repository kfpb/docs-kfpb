<div class="navbar navbar-inner block-header">
	<div class="muted pull-left">Usulan Dokumen In Proses Kembali Masuk ke MR (Hasil Koreksi/ACC)</div>
</div>
...
if ($e[udtgl_terima]==0000-00-00 AND $_SESSION[levelcv]<1 or $_SESSION[cv]=='1'){
...
<table width="100%" border=1>
	...
	<a title="Lampiran" href="https://view.officeapps.live.com/op/view.aspx?src=http://ekfpb.com/bdg/udmasuk/<?=$e[udfile];?>" target=_blank>Klik Disini (Jika Ada)</td></tr>
	</table>
	<br>
<table width="100%">
...
<b><u>File Usulan untuk dikoreksi/acc dari SPD-MR : <a href='https://view.officeapps.live.com/op/view.aspx?src=http://ekfpb.com/bdg/konsep_kirim/<? echo"$edf[disfile]";?>' target=_blank>klik disini (jika ada)</a></b></u>
...
$pds = mysql_query("SELECT a.*,
					...
					FROM uddis a WHERE a.uid='$_GET[id]' AND a.pId='49'  OR a.pId='21' ORDER BY a.pudid DESC");
...
while ($t=mysql_fetch_array($pds)){
	...
	if ($t[psACC]=="N"){
		...
		echo "<tr class='info'>
				...
				<td>$t[kepada] ($t[kepadajab])</td>
				<td>$t[pInstruksi]</td>
				<td>$t[info]<br>File Kembali : <a href='https://view.officeapps.live.com/op/view.aspx?src=http://ekfpb.com/bdg/jwb_usulandok/$t[filedis]' target=_blank>klik disini (jika ada)</a></td>
				<td><b>Tgl Baca:</b><br> $tglBaca<br><b>Tgl Slesai:<br></b> $tglSelesai</td>
			 </tr>";
	}
}
...
$q=mysql_query("UPDATE uddis SET psTglbaca = '$tgl',
 ...
 if ($q){
	 echo "<script>window.alert('Usulan Dokumen Kembali ke SPD');window.location=('../home.php?pages=usulandok')</script>";
  }else{
	  echo "<script>window.alert('Data Gagal Tersimpan');self.history.back();</script>";
  }
?>
...
<div class="span12">
	...
	<?php
		...
		$udmasuk = mysql_query("SELECT a.*, b.cNama, b.cIdjab FROM udokumen a, users b WHERE a.udpengusul=b.cId AND a.udstatus1='N' ORDER BY a.udtgl DESC");	
		...
		if ($s[udstatus1]='N'){
			...
			echo "<tr>";
				...
				echo "<td>$s[udstatus1]</td><td>";echo tgl_indo1($s[udtgl_kembali]);echo"</td><td class='center'>";
			...
			echo "
				...
				echo"<a href='?pages=usulandok&act=edit&id=$s[uid]'> <i class='icon-edit'></i> <a href='home.php?pages=usulandok&act=detail&id=$s[uid]' title=Detail' class='btn btn-info'> Baca/Detail </a>
				</td>
				</tr>";	
		}
	?>
