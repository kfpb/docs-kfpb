...
$q=mysql_query("UPDATE tdis SET    urut = '$_POST[urut]',
                                   ptgl   = '$_POST[ptgl]',
 ...
 $cc = mysql_fetch_array(mysql_query("SELECT * FROM tdis WHERE pdid='$_GET[id]'"));
 
  if ($q2){
	  	    
	  echo "<script>window.alert('Telaahan Sukses di Edit');window.location=('../../home.php?pages=tuin&act=detail&id=$cc[iid]')</script>";
  }else{
	 echo "<script>window.alert('Telaahan di edit');window.location=('../../home.php?pages=tuin&act=detail&id=$cc[iid]')</script>";
  }
