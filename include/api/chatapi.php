...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($httpCode !== 200) {
        ...
        http_response_code(500);
        // Ganti operator ?? dengan operator ternary yang kompatibel PHP 5.6
        $errorDetails = json_decode($n8nResponse, true); 
        echo json_encode(['error' => 'Chatbot mengembalikan error: ' . (isset($errorDetails['message']) ? $errorDetails['message'] : $n8nResponse)]);
        ob_end_flush(); 
        exit();
    }

   // Asumsi: $n8nResponse sekarang adalah string JSON seperti '{"output":"Halo!..."}' dari N8n.
$decodedN8nResponse = json_decode($n8nResponse, true); // Lakukan JSON decode pertama

$botResponseText = '';
// Periksa apakah hasil decode adalah array dan memiliki kunci 'output'
if (is_array($decodedN8nResponse) && isset($decodedN8nResponse['output'])) {
    $botResponseText = $decodedN8nResponse['output']; // Ambil teks dari kunci 'output'
} else {
    // Jika respons N8n bukan JSON dengan kunci 'output', gunakan respons mentah
    // Ini sebagai fallback jika N8n kadang mengembalikan teks biasa
    $botResponseText = $n8nResponse; 
}

error_log("Respons dari Chatbot N8n (setelah decode): " . $botResponseText);

// Kirim respons chatbot kembali ke frontend
http_response_code(200);
// Encode HANYA teks respons ke dalam kunci 'reply'
