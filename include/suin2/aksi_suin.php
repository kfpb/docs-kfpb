...
if($_FILES['fupload']['size']<=$maxsize){
  ...
  if ($q){
	if($_SESSION[levelcv]==0){
	 echo "<script>window.alert('Data Tersimpan');window.location=('../../home.php?pages=suin2')</script>";
	} else {
		echo "<script>window.alert('Data Tersimpan');window.location=('../../home.php?pages=usrtes')</script>";
	}
  }else{
...
elseif ($act=='tambahdisp'){
 ...
 if($_FILES['fupload']['size']<=$maxsize){
  ...
  {
	$q2=mysql_query("INSERT INTO pdiss(pNoagenda,ptgl,ptgls,pInstruksi,pSifat,pId,cId,iid,psACC,jawab,temuan,syarat,kondisi,gap,rootcause) 
	VALUES ('$_POST[noagenda]','$_POST[tglm]','$_POST[tgls]','$_POST[isi]','$_POST[sifat]','$_POST[pendisposisi]','$cid','$_GET[iid]','N','$_POST[jawab]','$_POST[temuan]','$_POST[syarat]','$_POST[kondisi]','$_POST[gap]','$_POST[rootcause]')"); 
...
*/
  echo "<script>window.alert('CAPA telah dibuat!');window.location=('../../home.php?pages=usrtes')</script>";	  	
