...
<?php
	...
	$stokmasuk = mysql_query("SELECT sg.nama, sg.satuan, sg.kode_barang as kodebrg, sg.keterangan, sg.jumlah, count(tr.stok_keluar) as stok_keluar, tr.stok_masuk, bt.jumlah as pesan, sp.siid, sp.pr, sp.sinmr
    FROM stok_gudang_barang_teknik sg, transaksi_stok_teknik tr, barang_teknik bt, spptek sp WHERE sg.kode_barang=bt.kode AND sg.kode_barang=tr.kode_barang AND bt.idspptek=sp.siid GROUP BY tr.kode_barang, sp.sinmr
	                        ");
...
$e = mysql_fetch_array(mysql_query("SELECT * FROM stok_gudang_barang_teknik WHERE kode_barang='$_GET[kode]'"));
$ef = mysql_fetch_array(mysql_query("SELECT a.*, b.cNama FROM spptek a,users b WHERE a.sipengirim=b.cId AND a.siid='$_GET[id]'"));
?>
<form method="post" action="include/spptek/aksi_sinter.php?act=editbaranggudang&id=<?=$e[id_stok_barang];?>" enctype="multipart/form-data" class="form-horizontal">
...
$data = mysql_fetch_array(mysql_query("SELECT * FROM stok_gudang_barang_teknik WHERE kode_barang='$_GET[kode]'"));
?>
...
<input type="hidden" name="nomor" value="<? echo "$newID" ?>">
  ...
  <!--      <div class="controls"><input class="input-large focused" id="kode_barang" type="text" name="kode_barang" value="<?php //echo $data[kode_barang] ?>"></div>-->
  <!--  </div>-->
    
    <div class="control-group">
    	<label class="control-label" for="kode_barang">Kode Barang</label>
        <div class="controls">
            <input type="text" placeholder="Pilih Kode / Tulis Kode" name="kode_barang" id="kode_barang" list="kd" value="<?php echo $data[kode_barang]; ?>">
            <datalist id="kd">
               	<?php
					$bt = mysql_query("SELECT * FROM barang_teknik");
					while ($bth=mysql_fetch_array($bt)){ ?>
...
elseif($_GET[act]=="detailbarang"){
	...
	$tr     = mysql_query("SELECT a.kode_barang, a.nama, b.keterangan, a.satuan, a.id_stok_barang, b.stok_masuk, b.stok_keluar, b.id_transaksi, b.tanggal, b.sinmr
    FROM stok_gudang_barang_teknik a,transaksi_stok_teknik b WHERE a.kode_barang=b.kode_barang AND a.kode_barang='$_GET[kode]'");
	$stok   = mysql_fetch_array(mysql_query("SELECT * FROM stok_gudang_barang_teknik WHERE kode_barang='$_GET[kode]'"));
...
<legend>Detail Transaksi Perbaikan-Pembelian Barang <?= $stok[nama];?> (SPPTek)</legend>
 <?php
	...
	?>
	<button class="btn-info btn-large" onclick="window.location.href='?pages=barangtek&act=tmbtransaksi&kode=<?php echo $stok[kode_barang] ?>'">Tambah Transaksi Gudang</button><br /><br />
	<?php
	}
	?>
	...
	while($s = mysql_fetch_array($tr)) {?>
		<tr>
                ...
                <td class='center'>
                    ...
                    <a href='include/spptek/aksi_sinter.php?act=hapustransaksibarang&id=<?php echo $s[id_transaksi] ?>&kode=<?php echo $s[kode_barang]?>' onClick=\'return confirm("Yakin ingin menghapus??")\' class='btn btn-warning'>Hapus</a>
                </td>
        </tr>
	...
	<?
		...
		while($s = mysql_fetch_array($stokmasuk)) {
			echo "<tr>
                ...
                <td>$s[kodebrg]</td>
                <td>$s[nama]</td>
                <td>$s[jumlah] <small>($s[satuan])</small></td>
                <td>$s[stok_keluar] <small>($s[satuan])</small></td>
                <td>$s[pesan] <small>($s[satuan])</small></td>
                <td>No PR : $s[pr]<br>Ket : $s[keterangan]</td>
                ...
                <td class='center'>
                    ...
                    <a href='home.php?pages=barangtek&act=editstokbarangtek&kode=$s[kodebrg]' class='btn btn-success'>Ubah</a>
