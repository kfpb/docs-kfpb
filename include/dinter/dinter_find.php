...
<legend>List Penerima Dokumen</legend>
		 ...
		 <br><form method="post" action="include/dinter/aksi_dinter.php?act=lp1&id=<?=$_GET[id];?>" enctype="multipart/form-data" class="form-horizontal">
			...
			<select multiple="multiple" id="dsin" name="dsin[]" class="chzn-select span8">
				...
				$cv = mysql_query("SELECT cId, cNama, bagian FROM users WHERE cId NOT IN(SELECT cId FROM dsin WHERE suid='$_GET[id]')");
				while ($dcv=mysql_fetch_array($cv)){
...
<center><strong><h4>DAFTAR INDUK DOKUMEN (ALL)</h4></strong></center>
	...
	<tbody>
		...
		echo"   <tr>
	            ...
	            echo"-<a title='File Dokumen' href='dok/<?=$e[jenisdok];?>/<?=$e[difile];?>' target='_blank'>File </a>"; }
				echo"</td>
				</tr>";	
		$no++;
		}
...
<table width="100%" border=1>
    ...
    <?php /*<tr><td>File Dokumen </td><td>: <a title='File Dokumen' href='dok/<?=$e[jenisdok];?>/<?=$e[difile];?>' target='_blank'>Klik Disini </a></td></tr>
...
?>
	...
	<table>
        ...
        // $dataDitemukan = true; 
        // if ($dataDitemukan) {
        //     $src = "dok/web/viewer.html?file=index1.php?id={$e['suid']}";
        // } else {
        //     $src = "dok/web/viewer.html?file=/dok/{$e['jenisdok']}/{$e['difile']}";
        // }
        ?>
...
<div class="span12">
    ...
    if (!empty($_POST['judul'])) {
        ...
        if ($_SESSION['cv'] == 0 || $_SESSION['cv'] == 1 || $_SESSION['cv'] == 53 || $_SESSION['cv'] == 50 ) {
            $query = "SELECT * FROM dinter 
                      ...
                      ORDER BY dikodok DESC";
        } else {
            $query = "SELECT * FROM dinter 
                      WHERE dipjdok = '" . mysql_real_escape_string($_SESSION['cv']) . "' 
                        AND (dijudok LIKE '%$judul%' OR dikodok LIKE '%$judul%') 
                      ORDER BY dikodok DESC";
        }

        $dist = mysql_query($query);
        $tampil = mysql_num_rows($dist);

        if ($tampil > 0) {
            echo "Ditemukan = <b>$tampil</b> dokumen dengan kata kunci kode/judul dokumen = <b>" . htmlspecialchars($_POST['judul']) . "</b><br><br>";
        } else {
            echo "Tidak ada dokumen ditemukan dengan kata kunci kode/judul dokumen = <b>" . htmlspecialchars($_POST['judul']) . "</b>.<br>";
            if ($_SESSION['cv'] != 0 && $_SESSION['cv'] != 1 && $_SESSION['cv'] != 53 && $_SESSION['cv'] != 50) {
                echo "Pastikan Anda mencari dokumen yang sesuai dengan hak akses Anda.";
            }
        }
    } else {
        echo "Masukkan kata kunci untuk pencarian.";
    }

    ?>
    <!--Ditemukan = <b><?php //echo $tampil;?></b> dokumen dengan kata kunci kode/ judul dokumen =<b><?php //echo $_POST[judul]; ?></b><br><br>-->
			<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="Tb14">
	<thead>
		<tr>
			<th>Kode</th>
	...
	<tbody>
		...
		echo"   <tr>
                <td>$s[dikodok]</td>
				<td>$s[dijudok]</td>
				<td><a href='?pages=dinterfind&act=lp&id=$s[suid]' class='btn btn-info'>List</a></td>";
                // <td><a href='dok/$s[jenisdok]/$s[difile]'class='btn btn-info' target=_blank>File</a></td>";
				// <td><a href='fdok/index1.php?id=$s[suid]'class='btn btn-info' target=_blank>File</a></td>";
				echo "
				<td class='center'>
				<a href='home.php?pages=dinterfind&act=detail&id=$s[suid]' title='Detail Info Dokumen' class='btn btn-info'>Detail</a>
				</td>
				</tr>";	
		}
