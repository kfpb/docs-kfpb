...
<?
	$e = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT a.*, b.cNama, b.cIdjab FROM dinter a,users b WHERE a.dipengirim=b.cId AND a.suid='$_GET[id]'"));
	$ef = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT a.*, b.cNama, b.cIdjab FROM dinter a,users b WHERE a.dipengirim=b.cId AND a.suid='$_GET[id]'"));
	$efg = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nama_jendok FROM jendok WHERE id_jendok='$ef[jenisdok]'"));

    $dok = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM dokumen WHERE kode_dok='$e[dikodok]'"));
...
<?php
	$psn = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT a.cUser,a.cNama,a.cIdjab, a.cFoto,b.tgl_baca, b.copyke FROM users a
						LEFT JOIN dsin b ON b.cId=a.cId
						WHERE b.suid='$_GET[id]' ORDER BY b.copyke ASC");
	$psn1 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT tgl_bls FROM dsin WHERE suid='$_GET[id]'");
	while ($t=mysqli_fetch_array($psn)){
		$j++;
...
**) Jika dokumen revisi lama dikembalikan ketika distribusi dokumen diisi = 1 dan langsung dimusnahkan oleh MR, jika dokumen revisi lama akan dikembalikan pada hari yang lain atau akan dimusnahkan sendiri oleh bagian maka diisi = 2<br>
