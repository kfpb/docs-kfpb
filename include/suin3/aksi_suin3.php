...
if($_FILES['fupload']['size']<=$maxsize){
  $q=mysql_query("INSERT INTO issurat(inmr,
                                   itgl,
                                   ipengirim,
                                   ikepada,
                                   ikepada2,
  ...
  if ($q){
	if($_SESSION[levelcv]==0){
	 echo "<script>window.alert('Data Tersimpan');window.location=('../../home.php?pages=suin3')</script>";
		...
		echo "<script>window.alert('Data Tersimpan');window.location=('../../home.php?pages=usrte3')</script>";
	}
  }else{
...
if (empty($lokasi_file)){
  ...
  $q=mysql_query("UPDATE issurat SET inmr     	= '$_POST[nomor]',
								   itgl	 	= '$_POST[tgl]',
								   ipengirim 	= '$_POST[pengirim]',
                                   ikepada	 = '$_POST[kepada]',
                                   ikepada2	 = '$_POST[kepada2]',
                                   ikepada3	 = '$_POST[kepada3]',
                                   ikepada4	 = '$_POST[kepada4]',
                                   iperihal	 = '$_POST[perihal]',
								   jenismss	 = '$_POST[jenismss]',
								   iket		 = '$_POST[ket]',
								   istatus2	 = '$_POST[status2]',
	  ...
	  UploadSMasuk($nama_file_unik);
$data=mysql_fetch_array(mysql_query("SELECT ifile,iid FROM issurat WHERE iid='$_GET[id]'"));
...
elseif ($act=='hapus'){
 $data=mysql_fetch_array(mysql_query("SELECT ifile,iid FROM issurat WHERE iid='$_GET[id]'"));
  ...
  mysql_query("DELETE FROM issurat WHERE iid='$_GET[id]'");
...
*/
  echo "<script>window.alert('Disposisi terkirim!');window.location=('../../home.php?pages=usrte3')</script>";	  	
...
elseif($act=='editdisp'){
  ...
  if ($q1){
	  ...
	  echo "<script>window.alert('Disposisi terkirim');window.location=('../../home.php?pages=suin3')</script>";
  }else{
