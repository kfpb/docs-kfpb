	...
	<div class="control-group">
        ...
        <div class="controls">
        	<select multiple="multiple" id="dosin" name="dosin[]" class="chzn-select span8">
            	<?php
				$cv = mysql_query("SELECT cId, cNama, bagian, cJabatan FROM users WHERE level='' AND cId IN(SELECT cId FROM dosin WHERE suid='$_GET[id]')");
				while ($dcv=mysql_fetch_array($cv)){
	    	     	echo "<option value='$dcv[cId]' selected>$dcv[cNama] ($dcv[cJabatan])</option>";
				}
				$cv = mysql_query("SELECT cId, cNama, bagian, cJabatan FROM users WHERE level='' AND cId NOT IN(SELECT cId FROM dosin WHERE suid='$_GET[id]')");
				while ($dcv=mysql_fetch_array($cv)){
	...
	<div class="control-group">
        ...
        <div class="controls">
        	<select multiple="multiple" id="dosin" name="psin[]" class="chzn-select span8">
            	<?php
				$cv = mysql_query("SELECT cId, cNama, bagian, cJabatan FROM users WHERE cId IN(SELECT cId FROM dosin WHERE suid='$_GET[id]')  AND level==''");
				while ($dcv=mysql_fetch_array($cv)){
	    	     	echo "<option value='$dcv[cId]' selected>$dcv[cNama] ($dcv[cJabatan])</option>";
				}
				$cv = mysql_query("SELECT cId, cNama, bagian, cJabatan FROM users WHERE cId NOT IN(SELECT cId FROM dosin WHERE suid='$_GET[id]')  AND level==''");
				while ($dcv=mysql_fetch_array($cv)){
	    	     	echo "<option value='$dcv[cId]'>$dcv[cNama] ($dcv[cJabatan])</option>";
				}
				?>                             
...
//	}
	...
	<?php
	if($_SESSION[levelcv]==0 OR $_SESSION[cv]==81){
