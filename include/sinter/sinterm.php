...
<fieldset>
<legend>Buat-Daftar Memo PR oleh Anda dan Bawahan Anda</legend>
<?
      ...
      <div class="control-group">
		...
		<select id="pengirim" class="chzn-select" name="pengirim">
		 ...
		 <select id="pengirim2" class="chzn-select" name="pengirim2" required>
			...
			<option value='2' >Bagian Support Plant</option>
			<option value='3' >Bagian Pemastian Mutu</option>
			<option value='26' >Bagian Produksi</option>
			<option value='11' >Bagian Teknik Pemeliharaan</option>
		</select>";
         ?> 
        </div> 
    </div>
...
<table border="1" cellpadding="1" cellspacing="1" width=100%>
	<tbody>
		...
		<tr>
			...
			<td style="text-align:center"Tulis Barang/Jasa atau <br>Upload file/screenshot PR sebagai Attachment </td>
			<td style="text-align:center">&nbsp;</td>
			<td style="text-align:center">&nbsp;</td>
		</tr>
	</tbody>
   ...
   <div class="control-group">
        ...
        <div class="controls">
        	<input class="input-file uniform_on" id="fileInput" type="file" name="fupload"> Max. 15 MB (nama file jangan ada spasi)<br>(Jika lampiran lebih dari 2, scan/satukan jadi 1 file PDF/ZIP)
        </div>
    </div>
...
<fieldset>
<legend>Buat Memo Angkutan Dan Asuransi</legend>
...
<table width="100%" border=1>
	...
	<tr><td>Konsep dari</td><td>: <strong><?=$ef[cNama];?> (<?=$ef[cJabatan];?>)</strong></td></tr>
    <tr><td>ACC 1</td><td>: <strong><?=$e[cNama];?> (<?=$e[cJabatan];?>)</strong></td></tr>
    <tr><td>ACC 2</td><td>: <strong><?=$efgh[cNama];?> (<?=$efgh[cJabatan];?>)</strong></td></tr>
...
<div>
	...
	$smasuk = mysql_query("SELECT * FROM sinter WHERE sipengirim=$_SESSION[cv] OR sipengirim1=$_SESSION[cv] OR sipengirim2=$_SESSION[cv] AND accsipengirim1='Y' AND sstatus='N' ORDER BY sitgl DESC");
...
<b>Keterangan : Highlight hijau = belum acc-untuk di ACC, Highlight merah = belum acc, ada komentar dari atasan, pindahkan pertanyaan atasan ke jawaban dalam kurung.</b></center>
	...
	<thead>
		<tr>
            ...
            <th width=40%>Isi Barang/Jasa</th>
			<th>Aksi</th>
		</tr>
	</thead>
	<tbody>
	<?
		...
		while($s = mysql_fetch_array($smasuk)) {
			...
			echo "<td>$s[sstatus]</td>
				  ...
				  <td><font size=2>$s[siket]<b>Link Lampiran : <a href='sinternal/$s[sifile]' target=_blank>Klik Disini/Detail!</a> </b><br><b>Note User :</b>$s[sikomen]<br><b>Note Atasan :</b> $s[sikomen2]</font><br><a href='home.php?pages=sinterm&act=detail&id=$s[siid]' class='btn btn-info'>Detail</a></td>";
			}    
			...
			echo "<td><font color=red>$s[sstatus]</font></td>
				  ...
				  <td><font size=2 color=red><b>Link Lampiran : $s[siket]<b>Link Lampiran : <a href='sinternal/$s[sifile]' target=_blank>Klik Disini/Detail!</a> </b><br><b>Note User :</b>$s[sikomen]<br><b>Note Atasan :</b> $s[sikomen2]</font><br><a href='home.php?pages=sinterm&act=detail&id=$s[siid]' class='btn btn-info'>Detail</a></td>";
			...
			<form method='post' action='include/sinter/aksi_sinter.php?act=edit3&id=$s[siid]'><input class='input-medium focused' type=text name=sikomen2 ><button class='btn btn-primary'>Komen</button></form><br>
					...
					{
			echo "<td><a href='include/sinter/aksi_sinter.php?act=acc2&id=$s[siid]' onClick=\"return confirm('Yakin akan ACC dan lanjut ke Atasan??')\" class='btn btn-info'>ACC !</a><br><br><br><form method='post' action='include/sinter/aksi_sinter.php?act=edit3&id=$s[siid]'><input type=text placeholder=isi_komen name=sikomen2 value='$s[sikomen2]' > <button class='btn btn-primary'>Komen</button></form></td>
			...
			<td class='center'><a href='home.php?pages=sinterm&act=detail&id=$s[siid]' class='btn btn-info'>Detail</a>
				...
				elseif ($s[sipengirim2]==$_SESSION[cv] AND $s[accsipengirim1]=='Y')
					{
			echo "<td><a href='include/sinter/aksi_sinter.php?act=acc4&id=$s[siid]' onClick=\"return confirm('Yakin akan ACC/kirim memo ini??')\" class='btn btn-info'>ACC !</a><br><br><br><form method='post' action='include/sinter/aksi_sinter.php?act=edit3&id=$s[siid]'><input type=text name=sikomen2 value='$s[sikomen2]'> <button class='btn btn-primary'>Komen</button></form></td>
			<td class='center'><a href='include/sinter/aksi_sinter.php?act=hapus&id=$s[siid]' onClick=\"return confirm('Yakin ingin menghapus??')\"><i class='icon-trash'></i></a>
				</td>";
					}
					else {
