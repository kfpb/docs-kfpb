...
<div>
<div class="span12">
<form method="post" action="?pages=usrdrvw">
<b>Pilih Bulan & Tahun : </b>
<select id="bulan" class="chzn-select span5" name="bulan" required="required">
                <option value=''><b>Bulan <? echo"$_POST[bulan]";?></b></option>
            	<option value='01'><b>Januari</b></option>
            	<option value='02'><b>Februari</b></option>
            	<option value='03'><b>Maret</b></option>
            	<option value='04'><b>April</b></option>
            	<option value='05'><b>Mei</b></option>
            	<option value='06'><b>Juni</b></option>
            	<option value='07'><b>Juli</b></option>
            	<option value='08'><b>Agustus</b></option>
            	<option value='09'><b>September</b></option>
            	<option value='10'><b>Oktober</b></option>
            	<option value='11'><b>November</b></option>
            	<option value='12'><b>Desember</b></option>
</select>
<select id="tahun" class="chzn-select span5" name="tahun" required="required">
                <option value=''><b>Tahun <? echo"$_POST[tahun]";?></b></option>
            	...
            	<option value='2026'><b>2025</b></option>
</select>
<input type=submit value=Cari />
	...
	if($data == null){
      ...
      $hasil  = mysql_query($cari);
        ...
        // 		$smasuk = mysql_query("SELECT a.*,b.*,c.cIdjab FROM dinter a LEFT JOIN dsin b ON a.suid=b.suid LEFT JOIN users c ON a.dipengirim=c.cId WHERE b.cId='$_SESSION[cv]' && a.distatus='Y' && a.dipjdok='$_SESSION[cv]' ORDER BY a.ditgl_review ASC");
		
		
	   //	$data = mysql_query("SELECT a.*,b.*,c.cIdjab datediff(a.ditgl_review,current_date()) as sisa FROM dinter a LEFT JOIN dsin b ON a.suid=b.suid LEFT JOIN users c ON a.dipengirim=c.cId  where a.dipjdok='$_SESSION[cv]' and datediff(a.ditgl_review,current_date()) BETWEEN 1 and 30 ORDER BY a.ditgl_review ASC");
        
	   	
		$no=1;
		while($s = mysql_fetch_array($hasil)) {
		    
		     $disposisidok = mysql_fetch_array(mysql_query("SELECT * FROM disposisidok where suid='$s[suid]'"));
		    if(empty($disposisidok)){
		...
		echo"   
				...
				if(empty($disposisidok)){
				echo "<td><center>
				<a href='home.php?pages=usrdin&act=detail&id=$s[suid]' title='Detail Dokumen' class='btn btn-info'> Detail</a>
				<a href='home.php?pages=usrdin&act=tambahdisp&id=$s[suid]' title='Review Dokumen' class='btn btn-info'> Review</a>
				</center>
				</td>";
				}else{
		        	echo"   <td><a href='home.php?pages=usrdin&act=detail&id=$s[suid]' title='Detail Dokumen' class='btn btn-info'> Detail</a></td>";
		    }
				echo"</tr>";	
		$no++;
		}
