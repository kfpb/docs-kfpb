...
?>
<? echo"<a href='home1.php?pages=usrtm1&act=detail&id=$_GET[id]' class='btn btn-info pull-right'><i class='icon-print'></i> Cetak</a>";?>
<strong>
...
<table width="100%" border=1>
	...
	$ds = mysql_query("SELECT * FROM disposisi WHERE siid='$e[siid]' AND dPendisposisi='$_SESSION[cv]'");
				$jr = mysql_num_rows($ds);
					...
					if ($jr<1){
						echo " <a href='?pages=usrtm1&act=tambahdisp&id=$e[siid]' class='btn btn-info'>Buat Disposisi</a>";
					}else{
						echo " <a href='?pages=usrtm1&act=editdisp&id=$e[siid]' class='btn btn-info'>Tambah Disposisi</i>";
					}
	?>
	</td></tr>
</table>
...
?>
<form method="post" action='home1.php?pages=usrtm2' target=_blank>
<input class="btn btn-primary" type="submit" value="Export Excel" /></form>
<div>
<div class="span12">
	...
	<tbody>
	<?php
		$smasuk = mysql_query("SELECT a.*,b.*,c.cIdjab FROM sinter a LEFT JOIN psin b ON a.siid=b.siid LEFT JOIN users c ON a.sipengirim2=c.cId  WHERE b.cId='$_SESSION[cv]' && a.sstatus='Y' ORDER BY a.sitgl DESC");
		...
		echo"<td width=5%>";echo tgl_indo($s[sitgl]);echo"</td>
                ...
                if ($s[tgl_baca]==0000-00-00) { echo "Belum Baca, klik Baca!";} else { echo tgl_indo($s[tgl_baca]); }
				echo"</td>
				<td width=5%><a href='home.php?pages=usrtm1&act=detail&id=$s[siid]' title=DetailMemo class='btn btn-info'>Baca!</a></td>
				</tr>";	
		}
		}
	?>
	...
	<h5>Baris Tabel Berwarna HIJAU = <strong>BELUM TERBACA OLEH ANDA MEMO</strong><br>
