<div class="navbar navbar-inner block-header">
	<div class="muted pull-left">Tembusan Memo/Undangan Masuk Internal untuk Anda</div>
</div>
...
?>
<? echo"<a href='home1.php?pages=usrt&act=detail&id=$_GET[id]' class='btn btn-info pull-right'><i class='icon-print'></i> Cetak</a>";?>
<strong>
<legend>Detail Tembusan Memo/Undangan Internal</legend>
<table width="100%" border=1>
	<tr><td width="24%">Nomor </td><td>: <?=$e[sinmr];?></td></tr>
    <tr><td>Tanggal </td><td>: <?=tgl_indo($e[sitgl]);?></td></tr>
	<tr><td>Memo/Undangan Dari</td><td>: <?=$ef[cJabatan];?> / <?=$e[cJabatan];?></td></tr>
	<tr><td>Tembusan untuk</td><td>: <? echo"$_SESSION[namacv]";?> (<? echo"$_SESSION[idjab]";?>)</td></tr>
	...
	<br></strong>
	<table width="100%">
    <tr><td align=top><font color=blue><b>Isi Memo/Undangan :</b></font></td><td></td></tr><tr><td><?=$e[siket];?></td></tr>
	<? echo"<tr><td colspan='2'><br><a href='home.php?pages=sinter&act=detail&id=$e[siid]' class='btn btn-info'>Klik Lihat Detail Penerima!</a> 
	...
	$ds = mysql_query("SELECT * FROM disposisi WHERE siid='$e[siid]' AND dPendisposisi='$_SESSION[cv]'");
				$jr = mysql_num_rows($ds);
					...
					if ($jr<1){
						echo " <a href='?pages=usrt&act=tambahdisp&id=$e[siid]' class='btn btn-info'>Buat Disposisi</a>";
					}else{
						echo " <a href='?pages=usrt&act=editdisp&id=$e[siid]' class='btn btn-info'>Tambah Disposisi</i>";
					}
	?>
...
$baca = mysql_fetch_array(mysql_query("SELECT * FROM tsin WHERE siid='$_GET[id]' AND cId='$_SESSION[cv]'"));
if ($baca[tgl_baca]=='0000-00-00') {
mysql_query("UPDATE tsin SET tgl_baca='$tgl_sekarang', sistatus='Y' WHERE siid='$_GET[id]' AND cId='$_SESSION[cv]'");
}
elseif  ($baca[tgl_baca]!='0000-00-00' AND $baca[sistatus]=='N') {
mysql_query("UPDATE tsin SET sistatus='Y' WHERE siid='$_GET[id]' AND cId='$_SESSION[cv]'");
}
...
<?php
    ...
    <div class="control-group">
		...
		/*$ef = mysql_fetch_array(mysql_query("SELECT a.*, b.cNama FROM tsin a,users b WHERE a.cId=b.cId AND a.siid='$_GET[id]'"));	*/
...
<div class="span12">
	...
	<tbody>
	<?php
		$smasuk = mysql_query("SELECT a.*,b.*,c.cIdjab, c.cJabatan FROM sinter a LEFT JOIN tsin b ON a.siid=b.siid LEFT JOIN users c ON a.sipengirim2=c.cId WHERE b.cId='$_SESSION[cv]' && a.sstatus='Y' ORDER BY a.sitgl DESC");
		...
		echo"<td width=8%>";echo tgl_indo($s[sitgl]);echo"</td>
				...
				echo"</td>
				<td width=5%><a href='home.php?pages=usrtt&act=detail&id=$s[siid]' title=DetailMemo class='btn btn-info'>Baca!</a></td>
				</tr>";	
		}
	?>
	</tbody>
	...
	<h5>Baris Tabel Berwarna HIJAU = <strong>BELUM TERBACA OLEH ANDA MEMO/ UNDANGAN</strong><br>
